...
{-
      ...
      let (Module _l _ _ _ decls) = modu
      let decl = last decls
          (SrcSpanInfo l _) = ann decl

      let forest = allocTokens modu toks
...
{- ++AZ++ working through
    ...
    it "gets the tokens for an added srcloc with one line spacing" $ do
      ...
      let (tm'',sigToks) = getTokensFor True tm' (ss2s ln)
      (drawTreeEntry tm'') `shouldBe`
            "((1,1),(32,18))\n|\n"++
            "+- ((1,1),(3,31))\n|  |\n"++
            "|  +- ((1,1),(1,24))\n|  |\n"++
    ...
    it "gets the tokens for an added indented srcloc" $ do
      ...
      let (tm1,declToks) = getTokensFor True forest (ss2s sspan)
      ...
      let (tm2,newSpan) = addToksAfterSrcSpan tm1 (ss2s sspan) (PlaceIndent 1 0 1) declToks
      (showGhc $ s2ss newSpan) `shouldBe` "foo:1048600:5-10"
    ...
    it "gets the tokens after adding and renaming" $ do
      ...
      let (tm4,toks4) = getTokensFor True tm''' (ss2s sspan3)
      ...
      -- are testing
      let (tm5,_sspan5,_tree5) = updateTokensForSrcSpan tm4 (ss2s sspan3) toks4
      ...
      let z = openZipperToSpan (ghcSrcSpanToForestSpan sspan3) $ Z.fromTree tm5
      (show $ treeStartEnd (Z.tree z))    `shouldBe` "(((ForestLine True 0 1 6),1),((ForestLine True 0 1 6),31))"
      (show (ghcSrcSpanToForestSpan sspan3)) `shouldBe` "(((ForestLine False 0 1 6),1),((ForestLine False 0 1 6),31))"
      (show $ treeStartEnd (Z.tree z) == (ghcSrcSpanToForestSpan sspan3)) `shouldBe` "True"

      let f1 = insertSrcSpan tm5 (ghcSrcSpanToForestSpan sspan3)
      ...
      let (forest',_tree) = getSrcSpanFor tm5 (ghcSrcSpanToForestSpan sspan3)
      ...
      let (tm6,_sspan6) = addToksAfterSrcSpan tm5 (ss2s sspan3) (PlaceIndent 1 0 2) declToks
      ...
      let (tm7,toks7) = getTokensFor True tm6 (ss2s sspan4)
    ...
    it "gets the tokens after renaming" $ do
      ...
      let (tm2,_sspan2,_tree2) = updateTokensForSrcSpan forest (ss2s sspan1) newToks
      (drawTreeEntry tm2) `shouldBe`
            "((1,1),(13,25))\n|\n"++
            "+- ((1,1),(6,23))\n|\n"++
      ...
      (show toks3') `shouldBe` "[((((0,2),(0,5)),ITvarid \"pow\"),\"pow\")]"
      let (tm3,newSpan3) = addToksAfterSrcSpan tm2 (ss2s sspan2) PlaceAdjacent toks3'
      (showSrcSpanF $ s2ss newSpan3) `shouldBe` "(((False,0,1,8),9),((False,0,1,8),12))"
      -- (show tm3) `shouldBe` ""
      ...
      let (tm4,toks4) = getTokensFor True tm3 (ss2s sspan3)
      (drawTreeEntry tm4) `shouldBe`
            "((1,1),(13,25))\n|\n"++
            "+- ((1,1),(6,23))\n|\n"++
            "+- ((10000000006,24),(10000000006,34))\n|\n"++
...
--
      ...
      let (tm3,_sspan3,_tree3) = updateTokensForSrcSpan tm2 (ss2s sspan2) newToks2
      (drawTreeEntry tm3) `shouldBe`
            "((1,1),(16,22))\n|\n"++
            "+- ((1,1),(11,32))\n|\n"++
            "+- ((10000000012,19),(10000000012,30))\n|\n"++
      ...
      let f1' = insertSrcSpan tm3 (ghcSrcSpanToForestSpan sspan4)
      -- (show f1) `shouldBe` ""
      (drawTreeEntry f1') `shouldBe`
            "((1,1),(16,22))\n|\n"++
            "+- ((1,1),(11,32))\n|\n"++
...
-}
      ...
      (show $ containsEnd (((ForestLine True 0 0 12),22),((ForestLine True 0 0 12),30)) (ghcSrcSpanToForestSpan sspan4)) `shouldBe` "False"
      (show $ containsMiddle (((ForestLine True 0 0 12),22),((ForestLine True 0 0 12),30)) (ghcSrcSpanToForestSpan sspan4)) `shouldBe` "True"

      (show $ containsEnd (((ForestLine False 0 0 12),25),((ForestLine False 0 0 16),22)) (ghcSrcSpanToForestSpan sspan4)) `shouldBe` "True"
      (show $ containsMiddle (((ForestLine False 0 0 12),25),((ForestLine False 0 0 16),22)) (ghcSrcSpanToForestSpan sspan4)) `shouldBe` "False"

      (show $ containsMiddle (((ForestLine False 0 0 12),25),((ForestLine False 0 0 12),28)) (ghcSrcSpanToForestSpan sspan4)) `shouldBe` "True"

      (show $ containsMiddle (((ForestLine True 0 0 12),29),((ForestLine True 0 0 12),37)) (ghcSrcSpanToForestSpan sspan4)) `shouldBe` "False"
      (show $ containsEnd    (((ForestLine True 0 0 12),29),((ForestLine True 0 0 12),37)) (ghcSrcSpanToForestSpan sspan4)) `shouldBe` "True"


      (show $ containsMiddle (((ForestLine False 0 0 12),32),((ForestLine False 0 0 16),22)) (ghcSrcSpanToForestSpan sspan4)) `shouldBe` "False"

      let (_b1,[_m1a,m1b],_e1) = splitSubtree tm3 (ghcSrcSpanToForestSpan sspan4)
      -- (show (b1,m1,e1)) `shouldBe` "([],[],[])"

      let (_b2,_m2,_e2) = splitSubtree m1b (ghcSrcSpanToForestSpan sspan4)
      -- (show (b2,m2,e2)) `shouldBe` "([],[],[])"


      -- let (f2,t2) = getSrcSpanFor tm3 (ghcSrcSpanToForestSpan sspan4)
      ...
      -}
      let ss2f@(_ss2fs,_ss2fe)        = ghcSrcSpanToForestSpan ss2
      let sspan4f@(_sspan4s,_sspan4e) = ghcSrcSpanToForestSpan sspan4
      ...
      let (tm5,toks5) = getTokensFor True tm3 (ss2s sspan4)
    ...
    it "gets tokens for a span should be same as getTokensForNoIntros" $ do
      ...
      let (tm',declToks) = getTokensFor True forest (ss2s sspan)
      (drawTreeEntry tm') `shouldBe`
            "((1,1),(13,16))\n|\n"++
            "+- ((1,1),(5,11))\n|\n"++
            "+- ((5,12),(5,19))\n|\n"++
      ...
      let (tm2,declToks2) = getTokensForNoIntros True forest (ss2s sspan)
      (drawTreeEntry tm2) `shouldBe`
            "((1,1),(13,16))\n|\n"++
            "+- ((1,1),(5,11))\n|\n"++
            "+- ((5,12),(5,19))\n|\n"++
      ...
      let (tm'',declToks') = getTokensFor True tm' (ss2s sspan2)
      (drawTreeEntry tm'') `shouldBe`
            "((1,1),(13,16))\n|\n"++
            "+- ((1,1),(5,11))\n|\n"++
            "+- ((5,12),(5,19))\n|\n"++
      ...
      let (tm3,declToks3) = getTokensForNoIntros True tm2 (ss2s sspan2)
      (drawTreeEntry tm3) `shouldBe`
            "((1,1),(13,16))\n|\n"++
            "+- ((1,1),(5,11))\n|\n"++
            "+- ((5,12),(5,19))\n|\n"++
  ...
  describe "getTokensBefore" $ do
    it "gets the tokens before a given srcloc" $ do
      ...
      let (tm',toksSpan) = getTokensFor True forest (ss2s sspan)
      ...
      let (tm'',toksBefore) = getTokensBefore tm' (ss2s sspan)
  ...
  describe "getSrcSpanFor" $ do
    it "inserts a SrcSpan if it was not in the forest" $ do
      ...
      let (forest',tree) = getSrcSpanFor forest (ss2f l)

      (showGhc l) `shouldBe` "test/testdata/TokenTest.hs:(19,1)-(21,13)"
      (showSrcSpan l) `shouldBe` "((19,1),(21,14))"
      (showForestSpan (ss2f l)) `shouldBe` "((19,1),(21,14))"
      (drawTreeEntry forest') `shouldBe`
            "((1,1),(21,14))\n|\n"++
            "+- ((1,1),(15,17))\n|\n"++
            "`- ((19,1),(21,14))\n"
...
{-
  describe "lookupSrcSpan" $ do
    it "looks up a SrcSpan that is fully enclosed in the forest" $ do
      ...
      (showGhc l) `shouldBe` "f:(19,1)-(21,13)"
      (showSrcSpan l) `shouldBe` "((19,1),(21,14))"
...
-}
  ...
  describe "insertSrcSpan" $ do
    it "checks that the forest is split into two parts" $ do
      ...
      --
      let z = openZipperToSpan (ss2f l) $ Z.fromTree forest
      let toksz = retrieveTokensInterim $ Z.tree z
      let (tokStartPos,tokEndPos) = forestSpanToSimpPos (ss2f l)
      ...
      let forest' = insertSrcSpan forest (ss2f l)
      (invariant forest') `shouldBe` []
      (drawTreeEntry forest') `shouldBe`
              "((1,1),(21,14))\n|\n"++
              "+- ((1,1),(15,17))\n|\n"++
    ...
    it "does not delete existing versioned spans" $ do
      ...
      let forest1 = insertSrcSpan forest (ss2f sspan)

      declToks <- liftIO $ tokenise (realSrcLocFromTok mkZeroToken) 0 True "where\n  sq = x ^ pow\n"
      -- putToksAfterPos ((6,21),(6,41)) at PlaceIndent 1 4 2
      let (forest2,_newSpan) = addToksAfterSrcSpan forest1 (ss2s sspan) (PlaceIndent 1 4 2) declToks
      ...
      let z = openZipperToSpan (ss2f sspan2) $ Z.fromTree forest2
...
-}
      ...
      (drawTreeEntry $ insertSrcSpan forest2 (ss2f sspan2)) `shouldBe`
               "((1,1),(13,25))\n|\n"++
               "+- ((1,1),(6,20))\n|\n"++
               "+- ((6,21),(6,41))\n|\n"++
               "+- ((1000007,5),(1000008,20))\n|\n"++
    ...
    it "insert a span after deleting one" $ do
      ...
      let (forest1,declToks) = getTokensFor True forest (ss2s sspan)
      -- (show forest1) `shouldBe` ""

      -- removeToksForPos ((10,22),(11,32))
      let sspan2 = posToSrcSpan forest ((10,22),(11,32))
      let (f2,_t2) = removeSrcSpan forest1 (ghcSrcSpanToForestSpan sspan2)
...
--
      let (_b1,_m1,_e1) = splitSubtree f2 (ghcSrcSpanToForestSpan sspan3)
      -- (show (b1,m1,e1)) `shouldBe` "([],[],[])"

--


      let (f3,_newSpan3,_tree3) = updateTokensForSrcSpan f2 (ss2s sspan3) declToks
    ...
    it "manipulates the Token Tree without breaking the invariant" $ do
      ...
      let (f2,_newSpan2,_tree2) = updateTokensForSrcSpan forest (ss2s sspan1) newToks1
      ...
      let (f3,_newSpan3,_tree3) = updateTokensForSrcSpan f2 (ss2s sspan2) newToks1
      ...
      let (f4,_toks4) = getTokensFor True f3 (ss2s sspan3)
      ...
      let (f5,_newSpan5,_tree5) = updateTokensForSrcSpan f4 (ss2s sspan3) newToks2
      ...
      let (f6,_newSpan6) = addToksAfterSrcSpan f5 (ss2s sspan5) PlaceAdjacent newToks3
      ...
      let (f7,_toks7) = getTokensFor True f6 (ss2s sspan6)
      ...
      let (f8,_newSpan8,_tree8) = updateTokensForSrcSpan f7 (ss2s sspan6) newToks2
      ...
      let (f9,_newSpan9) = addToksAfterSrcSpan f8 (ss2s sspan6) PlaceAdjacent newToks4
      ...
      let (f10,_toks10) = getTokensFor True f9 (ss2s sspan9)

--
      let z = openZipperToSpan (ss2f sspan9) $ Z.fromTree f9
      -- let (before,middle,end) = doSplitTree (Z.tree z) (fs sspan9)
      let (before1,middle,end) = splitSubtree (Z.tree z) (ss2f sspan9)
      ...
      -- (show (b2,m2,e2)) `shouldBe` ""
      let (b3,m3,e3) = splitSubtree (last middle) (ss2f sspan9)
      ...
      (show (containsStart (ss9) (ss2f sspan9),containsEnd ss9 (ss2f sspan9))) `shouldBe` "(False,True)"
...
--
    ...
    it "allocates comments in an if then else expression" $ do
      ...
      let t2 = insertSrcSpan t1 (ss2f s1)
      ...
      let t3 = insertSrcSpan t2 (ss2f s2)

-- innards of insertSrcSpan
      let sspan = ss2f s2
      let z = openZipperToSpan (ss2f s2) $ Z.fromTree t2
      (Z.isLeaf z) `shouldBe` True
...
--
      ...
      let t4 = insertSrcSpan t3 (ss2f s3)
      ...
      let (_,thenToks) = getTokensFor False t4 (ss2s s2)
      let (_,elseToks) = getTokensFor False t4 (ss2s s3)
  ...
  describe "removeSrcSpan" $ do
    it "removes a span from the forest" $ do
      ...
      let (forest'',delTree) = removeSrcSpan forest' (ss2f l)
      (drawTreeEntry forest'') `shouldBe`
              "((1,1),(21,14))\n|\n"++
               "+- ((1,1),(15,17))\n|\n"++
               "`- ((19,1),(21,14))(2,0)D\n" -- removed again
    ...
    it "removes a span and tokens without destroying the forest 1" $ do
      ...
      let sspan2 = posToSrcSpan forest ((9,1),(9,14))
      (showGhc sspan2) `shouldBe` "f:9:1-13"
      let (forest3,delTree) = removeSrcSpan forest2 (ss2f sspan2)
    ...
    it "removes a span and tokens without destroying the forest 2" $ do
      ...
      let tk2 = removeToksFromCache tk (ss2s sspan)
      ...
      let (tk3,_ss2') = putToksInCache tk2 (ss2s ss2) toks2
      ...
      (showRichTokenStream' $ retrieveTokensInterim $ getTreeFromCache ss2 tk3) `shouldBe`
      ...
      let (tk4,_ss3') = putToksInCache tk3 (ss2s ss3) toks3
      ...
      (showRichTokenStream' $ retrieveTokensInterim $ getTreeFromCache ss2 tk4) `shouldBe`
...
-}
    ...
    it "removes a where token followed by a comment" $ do
      ...
      let (f2,_f1') = removeSrcSpan forest (ghcSrcSpanToForestSpan sspan)
  ...
  describe "retrieveTokens" $ do
    it "extracts all the tokens from the leaves of the trees, in order" $ do
      ...
      (renderTree forest') `shouldBe` (showRichTokenStream' $ bypassGHCBug7351 toksClean)
...
-}
    ...
    it "retrieves the tokens in Ppr format LetStmt" $ do
      ...
      (showRichTokenStream' $ bypassGHCBug7351 toks) `shouldBe` "-- A simple let statement, to ensure the layout is detected\n\nmodule Layout.LetStmt where\n\nfoo = do\n        let x = 1\n            y = 2\n        x+y\n\n"
...
-}
  ...
  describe "updateTokensForSrcSpan" $ do
    it "replaces the tokens for a given span, inserting the span if needed" $ do
      ...
      let (forest',newSpan,_) = updateTokensForSrcSpan forest (ss2s l) (take 3 toks)
      -- (showGhc newSpan) `shouldBe` "f:(18,1)-(1000018,22)"
      (showGhc $ s2ss newSpan) `shouldBe` "foo:1073741843:1-22"
      (showSrcSpanF (s2ss newSpan)) `shouldBe` "(((True,0,0,19),1),((True,0,0,19),23))"
    ...
    it "replaces the tokens for a given span, and returns all the tokens later" $ do
      ...
      let (_tree,toksForOp) = getTokensFor True forest'' (ss2s l)
      ...
      let (forest''',_newSpan,_) = updateTokensForSrcSpan forest'' (ss2s l) newToks
    ...
    it "replaces a single token" $ do
      ...
      let (forest2,_ss1',_) = updateTokensForSrcSpan forest (ss2s ss1) ss1Tok
      ...
      let (forest3,_ss2',_) = updateTokensForSrcSpan forest2 (ss2s ss2) ss2Tok
...
-}
      ...
      let (forest4,_ss3',_) = updateTokensForSrcSpan forest3 (ss2s ss3) ss3Tok
    ...
    it "replaces a single token,without disturbing adjacent ones" $ do
      ...
      (showGhc ss2) `shouldBe` "f:33554445:16"
      (showSrcSpanF ss2) `shouldBe` "(((False,1,0,13),16),((False,1,0,13),17))"
      (show (ghcSrcSpanToForestSpan ss2)) `shouldBe` "(((ForestLine False 1 0 13),16),((ForestLine False 1 0 13),17))"
      let (tokStartPos,tokEndPos) = forestSpanToSimpPos (ghcSrcSpanToForestSpan ss2)
      ...
      let z = openZipperToSpan (ghcSrcSpanToForestSpan ss2) $ Z.fromTree f2
      let toksz = retrieveTokensInterim $ Z.tree z
      ...
      let fss = insertSrcSpan f2 (ghcSrcSpanToForestSpan ss2)
      ...
      let (forest',_tree) = getSrcSpanFor f2 (ghcSrcSpanToForestSpan ss2)
      (drawTreeEntry forest') `shouldBe`
               "((100000013,1),(100000013,21))\n|\n"++
               "+- ((13,1),(13,16))\n|\n"++
               "+- ((100000013,16),(100000013,17))\n|\n"++
      ...
      let (f3,_ss1',_) = updateTokensForSrcSpan f2 (ss2s ss2) toks2
  ...
  describe "replaceTokenForSrcSpan" $ do
    it "replaces a single token in a given span, without disturbing the tree" $ do
      ...
      let forest' = replaceTokenForSrcSpan forest (ss2s lt) newTok
...
-}
      ...
      let (f2,_toks1) = getTokensFor True f1 (ss2s ss1)

      let ss2 = posToSrcSpan f1 ((3,1),(3,31))
      let (f3,toks2) = getTokensFor True f2 (ss2s ss2)
      ...
      let (f4,ss4) = addToksAfterSrcSpan f3 (ss2s ss1) (PlaceAbsCol 2 1 0) toks2
{-
      (drawTreeEntry f4) `shouldBe`
              "((1,1),(32,18))\n|\n"++
              "+- ((1,1),(3,31))\n|  |\n"++
              ...
              "`- ((6,1),(32,18))\n"
-}
      (showSrcSpan $ s2ss ss4) `shouldBe` "((1048582,1),(1048582,31))"
      (showSrcSpanF $ s2ss ss4) `shouldBe` "(((False,0,1,6),1),((False,0,1,6),31))"
...
--
      ...
      let z = openZipperToSpan (ghcSrcSpanToForestSpan ss5) $ Z.fromTree f4

      (drawTreeEntry $ Z.tree z) `shouldBe`
                    "((1000006,1),(1000006,31))\n"
--
      let f5 = replaceTokenForSrcSpan f4 (ss2s ss5) newTok
{-
      (drawTreeEntry f5) `shouldBe`
              "((1,1),(32,18))\n|\n"++
              "+- ((1,1),(3,31))\n|  |\n"++
...
-}
  ...
  describe "insertNodeAfter" $ do
    it "adds a new SrcSpan after a specified one in the forest" $ do
      ...
      let newNode = Node (Entry (ss2f newSpan') NoChange toksNew) []
  ...
  describe "addToksAfterSrcSpan" $ do
    ...
    it "adds a new SrcSpan after an existing one in the forest 2" $ do
      ...
      (drawTreeEntry forest'') `shouldBe`
              ...
              "`- ((19,1),(21,14))\n"
      (showSrcSpanF $ s2ss sspan) `shouldBe` "(((False,0,1,17),1),((False,0,1,19),17))"
      (invariant forest'') `shouldBe` []
    ...
    it "adds a new SrcSpan after an existing one in the forest, with an indent" $ do
      ...
      let toks' = retrieveTokensInterim tree
      let (forest'',sspan) = addToksAfterSrcSpan forest' (ss2s l) (PlaceOffset 2 4 2) toks'
      (drawTreeEntry forest'') `shouldBe`
              "((1,1),(21,14))\n|\n"++
              "+- ((1,1),(15,17))\n|\n"++
              "+- ((19,1),(21,14))\n|\n"++
      ...
      (showSrcSpanF $ s2ss sspan) `shouldBe` "(((False,0,1,24),5),((False,0,1,26),18))"
      (invariant forest'') `shouldBe` []
...
-}
      ...
      let (forest'',sspan') = addToksAfterSrcSpan forest (ss2s sspan) (PlaceOffset 1 0 1) newToks
...
-}
      ...
      (showSrcSpanF $s2ss sspan') `shouldBe` "(((False,0,1,5),6),((False,0,1,5),13))"
      (invariant forest'') `shouldBe` []
      -- (show forest'') `shouldBe` ""

--
{-
      let (forest',tree) = getSrcSpanForDeep forest (ghcSrcSpanToForestSpan sspan)
...
-}
    ...
    it "adds a new SrcSpan after an existing one, with an indent based on whole prior line" $ do
      ...
      let (forest',_delTree) = removeSrcSpan forest (ghcSrcSpanToForestSpan sspan)
      (drawTreeEntry forest') `shouldBe`
              "((1,1),(8,6))\n|\n"++
               "+- ((1,1),(4,19))\n|\n"++
               "+- ((7,1),(7,6))(1,-5)D\n|\n"++
      ...
      let finsert = insertSrcSpan forest' (ghcSrcSpanToForestSpan sspan')
      ...
      let (forest'',sspan'') = addToksAfterSrcSpan finsert (ss2s sspan') (PlaceOffset 2 4 2) newToks
      (drawTreeEntry forest'') `shouldBe`
              "((1,1),(8,6))\n|\n"++
               "+- ((1,1),(4,19))\n|  |\n"++
               "|  +- ((1,1),(4,13))\n|  |\n"++
      ...
      (showSrcSpanF $ s2ss sspan'') `shouldBe` "(((False,0,1,6),5),((False,0,1,6),32))"
      -- (invariant forest'') `shouldBe` []
    ...
    it "adds a new SrcSpan after an existing one, with an indent catering for comments" $ do
      ...
      let (forest'',sspan'') = addToksAfterSrcSpan finsert (ss2s sspan') (PlaceIndent 1 0 2) newToks
      (drawTreeEntry forest'') `shouldBe`
              "((1,1),(17,23))\n|\n"++
               "+- ((1,1),(12,26))\n|  |\n"++
               "|  +- ((1,1),(11,15))\n|  |\n"++
      ...
      (showSrcSpanF $ s2ss sspan'') `shouldBe` "(((False,0,1,14),16),((False,0,1,14),21))"
      -- (invariant forest'') `shouldBe` []
    ...
    it "adds a new SrcSpan after deleting toks 1" $ do
      ...
      --  removeToksForPos ((7,1),(7,6))
      let sspan = posToSrcSpan forest ((7,1),(7,6))
      let (forest',_) = removeSrcSpan forest (ghcSrcSpanToForestSpan sspan)
      (drawTreeEntry forest') `shouldBe`
              "((1,1),(8,6))\n|\n"++
               "+- ((1,1),(4,19))\n|\n"++
               "+- ((7,1),(7,6))(1,-5)D\n|\n"++
      ...
      let (f,_t) = getSrcSpanFor forest' (ghcSrcSpanToForestSpan sspan')
      ...
      let (forest'',newSpan) = addToksAfterSrcSpan forest' (ss2s sspan') position newToks
      ...
      (showSrcSpanF $ s2ss newSpan) `shouldBe` "(((False,0,1,5),5),((False,0,1,5),32))"
      (invariant forest'') `shouldBe` []
    ...
    it "adds a new SrcSpan after an existing one, in a subtree." $ do
      ...
      let (f2,_newSpan,_oldTree) = updateTokensForSrcSpan forest' (ss2s sspan2) [tok2]
      (drawTreeEntry f2) `shouldBe`
               "((1,1),(13,25))\n|\n"++
               "+- ((1,1),(7,18))\n|  |\n"++
               "|  +- ((1,1),(6,20))\n|  |\n"++
      ...
      -- let (fss,_tree) = getSrcSpanFor f2 (ghcSrcSpanToForestSpan sspan3)
      let z = openZipperToSpan (ghcSrcSpanToForestSpan sspan3) $ Z.fromTree f2
      ...
      let contains zn = (startPos <= nodeStart && endPos >= nodeEnd)
            where
              (startPos,endPos) = treeStartEnd $ Z.tree zn
              (nodeStart,nodeEnd) = (ghcSrcSpanToForestSpan sspan3)

      (show $ filter contains childrenAsZ) `shouldBe` "[]"

--
      let z1 = openZipperToSpan (ss2f sspan3) $ Z.fromTree f2
      let (b1,m1,e1) = splitSubtree (Z.tree z1) (ss2f sspan3)
      (show (map treeStartEnd b1,map treeStartEnd m1,map treeStartEnd e1)) `shouldBe`
              "([(((ForestLine False 0 0 1),1),((ForestLine False 0 0 6),20))],"++
               "[(((ForestLine True 0 0 6),21),((ForestLine True 0 0 6),23)),"++
                "(((ForestLine False 0 0 6),26),((ForestLine False 0 0 7),18))],"++
               "[])"
      let (_b2,_m2,_e2) = splitSubToks (head m1) (ss2f sspan3)
      -- (show (b2,m2,e2)) `shouldBe` ""
--
      let fss = insertSrcSpan f2 (ghcSrcSpanToForestSpan sspan3)
      (drawTreeEntry fss) `shouldBe`
               "((1,1),(13,25))\n|\n"++
               "+- ((1,1),(7,18))\n|  |\n"++
               "|  +- ((1,1),(6,20))\n|  |\n"++
      ...
      let (forest'',sspan'') = addToksAfterSrcSpan f2 (ss2s sspan3) (PlaceOffset 1 4 2) newToks
      (drawTreeEntry forest'') `shouldBe`
               "((1,1),(13,25))\n|\n"++
               "+- ((1,1),(7,18))\n|  |\n"++
               "|  +- ((1,1),(6,20))\n|  |\n"++
      ...
      (showSrcSpanF $ s2ss sspan'') `shouldBe` "(((False,0,1,7),5),((False,0,1,10),7))"
      -- (invariant forest'') `shouldBe` []
    ...
    it "adds a new SrcSpan after the last one" $ do
      ...
      let (fwithspan,tree) = getSrcSpanFor forest (ghcSrcSpanToForestSpan sspan)
      ...
      let (forest',_sspan') = addToksAfterSrcSpan forest (ss2s sspan) (PlaceOffset 1 0 1) newToks
      (drawTreeEntry forest') `shouldBe`
              "((1,1),(3,18))\n|\n"++
              "+- ((1,1),(3,7))\n|\n"++
              "+- ((3,8),(3,18))\n|\n"++
    ...
    it "adds a SrcSpan after deleting, without extra tokens" $ do
      ...
      let (fwithspan,_t) = getSrcSpanFor forest' (ghcSrcSpanToForestSpan sspan')
      (drawTreeEntry fwithspan) `shouldBe`
              "((1,1),(17,23))\n|\n"++
               "+- ((1,1),(11,32))\n|  |\n"++
               "|  +- ((1,1),(11,27))\n|  |\n"++
      ...
      let z = openZipperToSpan (ghcSrcSpanToForestSpan sspan') $ Z.fromTree fwithspan
      let prevToks = retrievePrevLineToks z
      let prevToks'' = limitPrevToks prevToks (ss2s sspan')
      (show prevToks'') `shouldBe`
      ...
      (showSrcSpanF $ s2ss newSpan) `shouldBe` "(((False,0,1,12),26),((False,0,1,13),34))"
      (invariant forest'') `shouldBe` []
...
-}
      ...
      let (f2,_sspan1,_forest) = updateTokensForSrcSpan forest (ss2s sspan1) (map markToken toks1)
      (invariant f2) `shouldBe` []
...
-}
      ...
      let (f3,_sspan2,_f2) = updateTokensForSrcSpan f2 (ss2s sspan2) (map markToken toks2)
...
-}
      ...
      let (f4,_) = addToksAfterSrcSpan f3 (ss2s sspan3) PlaceAdjacent (map markToken toks3)
      (invariant f4) `shouldBe` []
    ...
    it "adds a SrcSpan, chasing a bug in MoveDef" $ do
      ...
      let (f1,_toks1) = getTokensFor True forest (ss2s sspan1)
      ...
      -- removeToksForPos ((24,5),(24,11))
      let (f2,_) = removeSrcSpan f1 (ghcSrcSpanToForestSpan sspan1)
      (drawTreeEntry f2) `shouldBe`
              "((1,1),(40,17))\n|\n"++
               "+- ((1,1),(23,8))\n|\n"++
               "+- ((24,5),(24,11))(2,-10)D\n|\n"++
      ...
      let (f3,_) = removeSrcSpan f2 (ghcSrcSpanToForestSpan sspan3)
      (drawTreeEntry f3) `shouldBe`
              "((1,1),(40,17))\n|\n"++
               "+- ((1,1),(23,8))\n|  |\n"++
               "|  +- ((1,1),(22,14))\n|  |\n"++
...
--
      let z = openZipperToSpan (ss2f sspan4) $ Z.fromTree f3
      (drawTreeEntry $ Z.tree z) `shouldBe`
              "((1,1),(40,17))\n|\n"++
               "+- ((1,1),(23,8))\n|  |\n"++
               "|  +- ((1,1),(22,14))\n|  |\n"++
      ...
      let (b1,m1,e1) = splitSubtree (Z.tree z) (ss2f sspan4)
      (show (map treeStartEnd b1,map treeStartEnd m1,map treeStartEnd e1)) `shouldBe`
              "([],"++
      ...
      let (b2,m2,e2) = splitSubtree (head m1) (ss2f sspan4)
      ...
      (containsStart ss (ss2f sspan4),containsEnd ss (ss2f sspan4)) `shouldBe` (True,False)

      let (sspanStart,_sspanEnd) = ss2f sspan4
      ...
      let (b3,m3,e3) = splitSubToks (head m2) (ss2f sspan4)
      (show (map treeStartEnd b3,map treeStartEnd m3,map treeStartEnd e3)) `shouldBe`
      ...
      let f3' = insertSrcSpan f3 (ss2f sspan4)
      (drawTreeEntry f3') `shouldBe`
              "((1,1),(40,17))\n|\n"++
              "+- ((1,1),(21,17))\n|\n"++
              "+- ((22,1),(24,11))\n|  |\n"++
      ...
      let (fwithspan,tree) = getSrcSpanFor f3 (ghcSrcSpanToForestSpan sspan4)
      (drawTreeEntry fwithspan) `shouldBe`
              "((1,1),(40,17))\n|\n"++
              "+- ((1,1),(21,17))\n|\n"++
              "+- ((22,1),(24,11))\n|  |\n"++
      ...
      let toks'' = placeToksForSpan fwithspan (ss2s sspan4) tree (PlaceOffset 2 0 2) newToks
      let (startPos,endPos) = nonCommentSpan toks''
      let newSpan = posToSrcSpan forest (startPos,endPos)
      (showGhc newSpan) `shouldBe` "f:24:1-6"

      let (forest',tree') = getSrcSpanFor f3 (ghcSrcSpanToForestSpan sspan4)
      -- (show tree') `shouldBe` ""
      ...
      let toks' = placeToksForSpan forest' (ss2s sspan4) tree' (PlaceOffset 2 0 2) newToks
      let newNode = Node (Entry (ghcSrcSpanToForestSpan newSpan') NoChange toks') []
...
--
      ...
      let (f4,_newSpan4) = addToksAfterSrcSpan f3 (ss2s sspan4) (PlaceOffset 2 0 2) newToks
      (drawTreeEntry f4) `shouldBe`
              "((1,1),(40,17))\n|\n"++
              "+- ((1,1),(21,17))\n|\n"++
              "+- ((22,1),(24,11))\n|  |\n"++
  ...
  describe "retrievePrevLineToks" $ do
    it "Retrieves the previous non-empty line tokens from an open zipper" $ do
      ...
      let z = openZipperToSpan (ghcSrcSpanToForestSpan tspan) $ Z.fromTree finsert
...
-}
  ...
  describe "splitSubToks" $ do
    it "splits sub tokens" $ do
      ...
      (showGhc $ s2ss $ forestSpanToSrcSpan (startPos,endPos)) `shouldBe` "foo:(6,16)-(8,18)"
  ...
  describe "syncAST" $ do
    it "updates an AST and a tree to have the same SrcSpan structure" $ do
      ...
      (drawTreeEntry forest'') `shouldBe`
              ...
              "`- ((19,1),(21,14))\n"
      (showSrcSpanF $ s2ss sspan) `shouldBe` "(((False,0,1,13),1),((False,0,1,15),17))"

      let decl' = syncAST decl (sf sspan) --  forest''
          forest''' = forest''
  ...
  describe "getTreeFromCache" $ do
    it "get the appropriate tree from the token cache, based on the SrcSpan" $ do
      ...
      let tk' = removeToksFromCache tk (ss2s l)
      ...
      let tid = treeIdFromForestSpan $ ghcSrcSpanToForestSpan sspan2
  ...
  describe "removeToksFromCache" $ do
    it "removes a SrcSpan from the main tree, and stashes it" $ do
      ...
      let tk' = removeToksFromCache tk (ss2s sspan)
      ...
      let (tk3,_newSpan) = putToksInCache tk' (ss2s sspan3) toks3
      (drawTokenCache tk3) `shouldBe`
               "tree TId 0:\n"++
               "((1,1),(18,23))\n|\n"++
               "+- ((1,1),(14,18))\n|\n"++
      ...
      let (forest2,tree1') = getSrcSpanFor tree1 (ghcSrcSpanToForestSpan sspan3)
  ...
  describe "formatAfterDelete" $ do
    it "does not leave a blank line in toks after deleting" $ do
      ...
      let (f2,_) = removeSrcSpan f1 (ss2f l)
...
{- Not sure this test is relevant anymore, as it does not start with a proper token tree
  ...
  describe "deleteGapsToks" $ do
    it "closes the gap when finding a Deleted Entry" $ do
      ...
      let (f2,_t2) = removeSrcSpan f1 (ghcSrcSpanToForestSpan sspan)
      ...
      let (f3,_newSpan) = addToksAfterSrcSpan f2 (ss2s sspan2) pos2 newToks
      (drawTreeEntry f3) `shouldBe`
            "((1,1),(16,23))\n|\n"++
            "+- ((1,1),(1,37))\n|\n"++
            "+- ((9,1),(13,23))\n|  |\n"++
...
--
      ...
      let (fwithspan,tree) = getSrcSpanFor f2 (ghcSrcSpanToForestSpan sspan2)

      let z = openZipperToSpan (ghcSrcSpanToForestSpan sspan2) $ Z.fromTree fwithspan
      let prevToks = case (retrievePrevLineToks z) of
                   RT [] -> reverseToks $ retrieveTokensInterim tree
                   xs -> xs

      let prevToks' = limitPrevToks prevToks (ss2s sspan2)
      ...
      let toks'' = placeToksForSpan fwithspan (ss2s sspan2) tree pos2 newToks
...
-}
  ...
  describe "calcEndGap" $ do
    it "Closes the gap when finding a Deleted Entry" $ do
      ...
      (calcEndGap f2 (ss2f sspan)) `shouldBe` (1,-14)
  ...
  describe "openZipperToSpan" $ do
    it "opens a zipper to a span, even if it has been added and is partly out of alignment" $ do
      ...
      let (layout2,newSpan) = addToksAfterSrcSpan layout (ss2s sspan) pos newToks

      (showGhc $ s2ss newSpan) `shouldBe` "foo:1048584:5-13"
